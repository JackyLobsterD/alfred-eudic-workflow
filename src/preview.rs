...
pub fn write_preview(
    ...
    // 🤖 LLM — rendered FIRST. The section body is a plain <div>; if
    // the LLM is still being fetched in a background subprocess we
    // emit a "Still thinking…" placeholder and a small JS poller (see
    // bottom of <body>) that XHR-fetches claude.html every 2 s and
    // swaps the div's innerHTML in once the finished content arrives.
    // No iframe (Quick Look's WebKit doesn't reliably honour iframe
    // meta-refresh) and no whole-page reload, so nothing else in the
    // card ever flashes.
    let want_llm = llm_loading
        || llm.map(|r| !render_claude_inner(r).is_empty()).unwrap_or(false);
    if want_llm {
        let inner_now = if llm_loading {
            // Marker text the JS poller uses to detect we're still
            // waiting. Kept in sync with `write_claude_html`'s loading
            // body so the same string identifies the loading state in
            // both the parent div and the fetched claude.html.
        ...
        let _ = write!(
            body,
            "<section><h2>🤖 LLM</h2><div id=\"llm-area\">{}</div></section>",
            inner_now,
        );
    }
    ...
    // JS poller: fetches claude.html every 2 s while the LLM is still
    // being generated by the background subprocess. As soon as the
    // fetched body no longer contains the "Still thinking…" marker, we
    // extract its <body> contents and swap them into #llm-area. The
    // rest of the card is never touched.
    //
    // We rely on XHR for file://-to-file:// requests in the same dir,
    // which Quick Look's WebKit allows. A cache-busting query string
    // prevents stale responses.
    let poller_js = if llm_loading {
        "\n<script>(function(){var area=document.getElementById('llm-area');\
if(!area)return;var done=false;\
function poll(){if(done)return;var x=new XMLHttpRequest();\
x.open('GET','claude.html?_='+Date.now(),true);\
x.onreadystatechange=function(){if(x.readyState!==4)return;\
if(x.status===200&&x.responseText.indexOf('Still thinking')===-1){\
var m=x.responseText.match(/<body[^>]*>([\\s\\S]*?)<\\/body>/i);\
if(m){area.innerHTML=m[1];done=true;return;}}\
if(!done)setTimeout(poll,2000);};\
x.onerror=function(){if(!done)setTimeout(poll,2000);};\
x.send();}\
poll();})();</script>"
    } else {
        ""
...
<style>{STYLE}</style></head><body><h1>{}</h1>{ph}{links}{body}{poller}</body></html>",
        esc(spell),
        links = links_html,
        poller = poller_js,
    );
